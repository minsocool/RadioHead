...
function SongList(props) {
  ...
  const [songs, setSongs] = useState([
    {
      name: 'Paella Valenciana, with rabbit and chicken; and seasong paella',
  ...
  const [categories, setCategories] = useState([
    {
      name: 'BBQ',
      url: 'https://images.songy.vn/BlogsContents/46444498_1785582584898023_6834569445101273088_n(1).jpg',
    },
    {
      name: 'Breakfast',
  ...
  const filteredSongs = () =>
    songs.filter(eachSong =>
      eachSong.name.toLowerCase().includes(searchText.toLowerCase()),
    );
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: 'white'}}>
      <View>
        ...
        {/* <ScrollView>
                {songs.map(eachsong => <songItem 
                    song = {eachsong} key={eachsong.name}/>)}                
            </ScrollView> */}
      </View>
      {filteredSongs().length > 0 ? (
        <FlatList
          ...
          keyExtractor={eachSong => eachSong.name}
        />
      ) : (
        <View
          style={{
